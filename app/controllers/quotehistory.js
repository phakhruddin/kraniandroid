...
function checkClick(e){
	Alloy.Globals.Log("quotehistory.js::transform is JSON.stringify(e) ::" +JSON.stringify(e));
}
...
function transformFunction(model) {
        ...
        ///Alloy.Globals.Log("quotehistory.js::transform is ::" +JSON.stringify(transform));
...
Alloy.Globals.Log("quotehistory.js::JSON stringify content: "+JSON.stringify(content));

function setDate(e){
	Alloy.Globals.Log("quotehistory.js::JSON stringify content: "+JSON.stringify(e));
	var date = e.value;
    ...
    Alloy.Globals.Log("quotehistory.js::setDate: date: "+date+" datesplit: "+datesplit+" datepaid: "+datepaid);
...
function jobDetailAddRow (date,notesbody,imageurl,dateadded,employee) {
		Alloy.Globals.Log("quotehistory.js::jobDetailAddRow: date: "+date+"  dateadded: "+dateadded+" new Date(+dateadded): "+new Date(+dateadded));
...
//Add row here.
var sid = args.sid;
Alloy.Globals.Log("quotehistory.js::sid right before key in contents value: "+sid);
Alloy.Globals.Log("quotehistory.js::content.length: "+content.length);
var paidamount = 0;
...
Alloy.Globals.Log("quotehistory.js::paidamount : "+paidamount);
//$.labor_table.appendRow(jobrowentry);

function closeWin(e) {
        Alloy.Globals.Log("quotehistory.js::e is: "+JSON.stringify(e));
}

function UploadPhotoToServer(imagemedia){
        Alloy.Globals.Log("quotehistory.js::UploadPhotoToServer:: Upload photo to the server.");
        ...
        Alloy.Globals.Log("quotehistory.js::beginning to upload to the cloud.");
       ...
       // uploadPictoGoogle(image,"uploadphoto3.jpeg");
        ...
        //Alloy.Globals.Log("quotehistory.js::UploadPhotoToServer::image sid is : " +imagesid);
}

function uploadFile(e){
        Alloy.Globals.Log("quotehistory.js::JSON stringify e uploadFile : " +JSON.stringify(e));
...
function takePic(e){ 
        Alloy.Globals.Log("quotehistory.js::JSON stringify e takePic:" +JSON.stringify(e));
        Titanium.Media.showCamera({
                success:function(e){
                        ...
                        Alloy.Globals.Log("quotehistory::takePic::error:unable to load the camera");
                        alert("error:"+e.code+": Unable to load camera.");
                }, cancel:function(e){
                		alert("error:"+e.code+": Unable to load camera.");
                        Alloy.Globals.Log("quotehistory::takePic::cancel:unable to load the camera");
                },
                allowEditing:true,
...
var sid = args.sid;
Alloy.Globals.Log("quotehistory.js::before notes_textarea hintText: JSON.stringify(args): "+JSON.stringify(args)+" sid:"+sid);
$.notes_textarea._hintText = "100";           
$.notes_textarea.addEventListener("blur",function(e){
        Alloy.Globals.Log("quotehistory.js::JSON.stringify(e)  :" +JSON.stringify(e));
        e.source.keyboardToolbar.items = null;
        var datepaid = e.source.datepaid;
        $.paymentdonebutton.datepaid = datepaid;
        if (datepaid) {
        	Alloy.Globals.Log("quotehistory.js::before enterNotes(e): JSON.stringify(e): "+JSON.stringify(e));
        	enterNotes(e);
        	e.source.value = "";
        } else {
...
$.paymentdonebutton.addEventListener("click",function(e){
    Alloy.Globals.Log("quotehistory.js::paymentdone:JSON.stringify(e)  :" +JSON.stringify(e));
 	$.notes_textarea.blur();
});
        
function enterNotes(e,imgurl) {
        Alloy.Globals.Log("quotehistory.js::JSON.stringify(e) enterNotes  :" +JSON.stringify(e));
        //$.quotehistory_window.show($.notes_textarea);
        //$.quotehistory_window.add(textfield);
        var date = new Date();
        var now = Date.now().toString();
        var jobitemid = now;
        Alloy.Globals.Log("quotehistory.js:: jobitemid is:"+jobitemid);
        ...
        payment.fetch();
        var content = payment.toJSON();
        Alloy.Globals.Log("quotehistory.js::JSON stringify payment after write: "+JSON.stringify(content));
        var thedate = date.toString().replace(".","").split(' ',4).toString().replace(/,/g,' ')+' '+Alloy.Globals.formatAMPM(date);
        //Alloy.Globals.Log("quotehistory.js::thedate is: " +thedate);
        ...
        Alloy.Globals.Log("quotehistory.js::enterNotes: paidamount , parseFloat(notesbody) : "+paidamount+" + "+parseFloat(notesbody));
 ...
 function submit(thedate,notesbody,imageurl,jobitemid,payment,employee) {  
    ...
    onload: function() {
        try {
	    		...
	    		Alloy.Globals.Log("quotehistory.js::submit: number of link found: " +link+ " length: "+link.length);
	    		...
	    		Alloy.Globals.Log("quotehistory.js::submit:: update DB with jobitemid :" +jobitemid);
    ...
    onerror: function(e) {
        Alloy.Globals.Log("quotehistory.js::submit::error e: "+JSON.stringify(e));
...
Alloy.Globals.Log("quotehistory.js::jsonargs : "+JSON.stringify(args));
...
Alloy.Globals.Log("quotehistory.js::value derived from args: proposalnumber: "+proposalnumber+" firstname: "+firstname+" lastname: "+lastname);
...
function getParentFolder(args) {
	...
	var xhr = Ti.Network.createHTTPClient({
	    ...
	    try {
	    		...
	    		Alloy.Globals.Log("quotehistory.js::args inside getParentFolder: "+JSON.stringify(args));
	    		//var filename = 'test03';
	...
	xhr.onerror = function(e){
		alert("error:"+e.code+": Please connect to the network.");
		Alloy.Globals.Log("quotehistory::getParentFolder::Unable to connect to the cloud. error is: "+JSON.stringify(e));
	};
...
function createSpreadsheet(filename,parentid) {
	Alloy.Globals.Log("quotehistory.js::create ss with filename: "+filename+" and parentid: "+parentid);
	var jsonpost = '{'
		 +'\"title\": \"'+filename+'\",'
		 +'\"shared\": \"true\",'
		 +'\"parents\": ['
	    ...
	    try {
	    		...
	    		var json = JSON.parse(this.responseText);
	    		var sid = json.id;
	    		Alloy.Globals.Log("quotehistory.js::sid : "+sid);
	...
	xhr.onerror = function(e){
		alert("error:"+e.code+": Please connect to the network.");
		Alloy.Globals.Log("quotehistory::createSpreadsheet::Unable to connect to the cloud. error is: "+JSON.stringify(e));
	};
	xhr.open("POST", 'https://www.googleapis.com/drive/v2/files');	
	xhr.setRequestHeader("Content-type", "application/json");
    xhr.setRequestHeader("Authorization", 'Bearer '+Alloy.Globals.googleAuthSheet.getAccessToken());
    Alloy.Globals.Log("quotehistory.js::json post: "+jsonpost);
	xhr.send(jsonpost);
}
...
function fileExist(){
	    ...
	    try {
			...
			Alloy.Globals.Log("quotehistory.js::jsonlist.items.length: "+jsonlist.items.length);
			var filename = Titanium.App.Properties.getString('filename');
			filelist = [];
			if (jsonlist.items.length == "0" ){
				Alloy.Globals.Log("quotehistory.js::File DOES NOT EXIST");
				var fileexist = "false";
				createSpreadsheet(filename,parentid);  // create file when does not exists
			} else {
				var fileexist = "true";
				Alloy.Globals.Log("quotehistory.js::quotehistory.js::fileExist:: File exist. sid is: "+jsonlist.items[0].id+" Skipped.");
				populatepaymentSIDtoDB(filename,sid);
			};
		}
		});
	xhr.onerror = function(e){
		alert("error:"+e.code+": Please connect to the network.");
		Alloy.Globals.Log("quotehistory::fileExist::Unable to connect to the cloud. error is : "+JSON.stringify(e));
	};
...
//Alloy.Globals.Log("quotehistory.js::create spreadsheet with filename: "+filename+" and parentid: "+parentid); 
//createSpreadsheet(filename,parentid); 
...
Alloy.Globals.Log("quotehistory.js::paymentsidfile" +paymentsidfile);
//Alloy.Globals.Log("quotehistory.js::JSON.stringify(paymentsidfilejson)" +paymentsidfilejson);*/
...
function uploadPictoGoogle(image,filename){
	Alloy.Globals.Log("quotehistory.js::uploadPictoGoogle::create ss with filename: "+filename);
	 		...
	 		var xhr =  Titanium.Network.createHTTPClient({
			    onload: function() {
			    	try {
	    				...
	    				Alloy.Globals.Log("quotehistory.js::uploadPictoGoogle::entering urlimage with info below e: "+JSON.stringify(e));
...
function shareAnyonePermission(sid){
	Alloy.Globals.Log("quotehistory.js::shareAnyonePermission::sid: "+sid);
	var jsonpost = '{'
		 +'\"role\": \"reader\",'
		 +'\"type\": \"anyone\"'
		+'}';
	...
	xhr.onerror = function(e){
		Alloy.Globals.Log("quotehistory::shareAnyonePermission::Unable to connect to the cloud. error is :"+JSON.stringify(e));
		alert("error:"+e.code+": Please connect to the network.");
	};
	xhr.open("POST", 'https://www.googleapis.com/drive/v2/files/'+sid+'/permissions');	
	xhr.setRequestHeader("Content-type", "application/json");
    xhr.setRequestHeader("Authorization", 'Bearer '+Alloy.Globals.googleAuthSheet.getAccessToken());
    Alloy.Globals.Log("quotehistory.js::shareAnyonePermission::json post: "+jsonpost);
	xhr.send(jsonpost);
}

var headertitle = args.title.title.split(':')[0];
Alloy.Globals.Log("quotehistorys:headerTitle:: " +headertitle);
//$.paymentsection.headerTitle = headertitle;

$.labor_table.addEventListener("delete", function(e){
	Alloy.Globals.Log("quotehistory.js::$.labor_table delete: "+JSON.stringify(e));
	...
	Alloy.Globals.Log("quotehistory.js::$.labor_table delete: idtag:"+existingurlsidtag+" selfhref: "+existingurlsselfhref+" edithref: "+existingurlsedithref);
	var xhr = Ti.Network.createHTTPClient({
	    onload: function(e) {
	    try {
	...
	//xhr.setRequestHeader("Content-type", "application/json");
    xhr.setRequestHeader("Authorization", 'Bearer '+Alloy.Globals.googleAuthSheet.getAccessToken());
	if (existingurlsedithref) {xhr.send();} else {Alloy.Globals.Log("quotehistory.js::$.labor_table delete: NO edithref. abort delete ");}
	Alloy.Globals.Log("quotehistory.js::$.labor_table delete: DONE: DELETE "+existingurlsedithref);
});

function blurIT(e) {
	Alloy.Globals.Log("quotehistory.js::blurIT::JSON.stringify(e): "+JSON.stringify(e));
	$.notes_textarea.blur();
}
...
function updateInvoice(paidamount,datepaid,balance){ 
    ...
    onerror: function(e) {
        Alloy.Globals.Log("quotehistory.js::updateInvoice::error e: "+JSON.stringify(e));
        alert("error:"+e.code+": Please connect to the network.");
    }
});
